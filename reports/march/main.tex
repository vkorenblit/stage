...
This will activate new functionalities such as enabling llvmpipe software rast
...
can be added to Buildroot. When this part of the project is achieved, the next step
is to create a cross-toolchain based on Clang to compile Buildroot componentes
supported by this front-end. \footnote{Mainline Linux kernel and glibc do not yet
compile with Clang}
...
that only the necessary targets are built.
...
After having investigated why Mesa3D was not able to link dinamically against
...
it must be linked with libLLVM.so (host-variant), so this library must be built too.
...
\subsection*{PATCH v3}
The PATCH v3 series sent to the Buildroot mailing list on the $9^{th}$ March
contains the following 6 commits:
\begin{itemize}
...
Aarch64), the following goal was to activate OpenCL support. This task involved
...
\begin{itemize}
  ...
  Drivers (hardware dependent userspace GPU drivers). Currently it supports OpenCL
  1.1 and it is close to 1.2. It was merged into Mesa in 2012 and has the following
  dependencies:
    \begin{itemize}
...
\begin{itemize}
  ...
  line containing the name of the shared library containing the implementation.
  \item \textbf{One or more OpenCL implementations (the ICDs)}: the shared libraries
  pointed to by the .icd files.
\end{itemize}
...
implementations of most library requiremnts, allowing targets to override them
...
generated with Buildroot must work directly after being generated. The next step
...
\centering
  ...
  \caption{OpenCL results in HTML}
...
\subsection*{PATCH v4}
The PATCH v4 series sent to the Buildroot mailing list on the $29^{th}$ March
contains the following 11 commits:
\begin{itemize}
...
\end{itemize}
After taking into account all these considerations, the next version of the series
was prepared.

\subsection*{PATCH v5}
The PATCH v5 series sent to the Buildroot mailing list on the $4^{th}$ April
contains the following 7 commits:
\begin{itemize}
  ...
  \item {[PATCH v4 4/7]} package/clang: new package
  \item {[PATCH v4 5/7]} package/libclc: new package
  \item {[PATCH v4 6/7]} package/mesa3d: enable OpenCL support
  \item {[PATCH v4 7/7]} package/clinfo: new package
\end{itemize}
